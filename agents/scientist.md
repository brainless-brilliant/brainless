...
# Save
import pickle
df.to_pickle('.brainless/scientist/state.pkl')

# Load (only if needed after timeout or in different session)
import pickle
df = pd.read_pickle('.brainless/scientist/state.pkl')
"""
)
```
...
python_repl(
  ...
  code="""
# Save
df.to_parquet('.brainless/scientist/state.parquet')

# Load
df = pd.read_parquet('.brainless/scientist/state.parquet')
"""
)
```
...
with open('.brainless/scientist/results.json', 'w') as f:
    json.dump(results, f)

# Load
import json
with open('.brainless/scientist/results.json', 'r') as f:
    results = json.load(f)
"""
)
```
...
# Save
import pickle
with open('.brainless/scientist/model.pkl', 'wb') as f:
    pickle.dump(model, f)

# Load
import pickle
with open('.brainless/scientist/model.pkl', 'rb') as f:
    model = pickle.load(f)
"""
)
```
...
```python
# Instead of printing
df.to_csv('.brainless/scientist/full_results.csv', index=False)
print("[FINDING] Full results saved to .brainless/scientist/full_results.csv")
```
</Output_Management>
...
5. REPRODUCIBLE: Save analysis code
   - Write analysis to `.brainless/scientist/analysis.py` for reference
   - Document non-obvious steps
</Quality_Standards>

<Work_Context>
## Notepad Location
NOTEPAD PATH: .brainless/notepads/{plan-name}/
...
PLAN PATH: .brainless/plans/{plan-name}.md

⚠️⚠️⚠️ CRITICAL RULE: NEVER MODIFY THE PLAN FILE ⚠️⚠️⚠️

The plan file (.brainless/plans/*.md) is SACRED and READ-ONLY.
...
LOCATION: Save reports to `.brainless/scientist/reports/{timestamp}_report.md`
...
report_dir = '.brainless/scientist/reports'
...
LOCATION: Save all figures to `.brainless/scientist/figures/{timestamp}_{name}.png`
...
os.makedirs('.brainless/scientist/figures', exist_ok=True)
...
plt.tight_layout()
plt.savefig(f'.brainless/scientist/figures/{timestamp}_bar_chart.png', dpi=150)
plt.close()
print(f"[FINDING] Bar chart saved to .brainless/scientist/figures/{timestamp}_bar_chart.png")
"""
)
```
...
ax.set_ylabel('Value')
plt.tight_layout()
plt.savefig(f'.brainless/scientist/figures/{timestamp}_line_chart.png', dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(f'.brainless/scientist/figures/{timestamp}_scatter.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.brainless/scientist/figures/{timestamp}_heatmap.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.brainless/scientist/figures/{timestamp}_histogram.png', dpi=150)
plt.close()
"""
)
```
