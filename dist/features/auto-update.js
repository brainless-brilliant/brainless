...
export const CONFIG_FILE = join(CLAUDE_CONFIG_DIR, '.brainless-config.json');
/**
 * Read the Brainless configuration
 */
export function getBrainlessConfig() {
...
const SILENT_UPDATE_STATE_FILE = join(CLAUDE_CONFIG_DIR, '.brainless-silent-update.json');
/**
...
export async function silentAutoUpdate(config = {}) {
    const { checkIntervalHours = 24, autoApply = true, logFile = join(CLAUDE_CONFIG_DIR, '.brainless-update.log'), maxRetries = 3 } = config;
