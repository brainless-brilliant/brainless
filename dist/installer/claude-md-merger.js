 ...
 */
export function detectExistingCLAUDEmd(cwd = process.cwd()) {
    const locations = [
 ...
 */
export function findPrimaryLocation(locations) {
 ...
 */
export async function promptUserForChoice(location, verbose = false) {
    ...
    return mapping[choice.trim()];
}
/**
 * Create backup of existing file
 */
export function backupExistingFile(filePath) {
 ...
 */
export async function mergeCLAUDEmd(userContent, pluginContent, verbose = false) {
...
- Confidence should reflect merge quality (aim for 80+)`;
        ...
        const changes = changesMatch
            ? changesMatch[1].split(',').map((c) => c.trim())
 ...
 * Main merge workflow
 */
export async function performCLAUDEmdMerge(pluginContent, targetPath, options = {}) {
...
//# sourceMappingURL=claude-md-merger.js.map