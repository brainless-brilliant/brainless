...
describe('delegation-enforcer', () => {
    ...
    beforeEach(() => {
        originalDebugEnv = process.env.AVK_DEBUG;
    });
    afterEach(() => {
        if (originalDebugEnv === undefined) {
            delete process.env.AVK_DEBUG;
        }
        else {
            process.env.AVK_DEBUG = originalDebugEnv;
        }
    });
    describe('enforceModel', () => {
        ...
        it('logs warning only when AVK_DEBUG=true', () => {
            const input = {
                description: 'Test task',
                prompt: 'Do something',
        ...
        it('does not log warning when AVK_DEBUG is false', () => {
            const input = {
                description: 'Test task',
                prompt: 'Do something',
                subagent_type: 'executor'
            };
            process.env.AVK_DEBUG = 'false';
            const result = enforceModel(input);
    ...
    describe('processPreToolUse', () => {
        ...
        it('logs warning only when AVK_DEBUG=true and model injected', () => {
            const toolInput = {
                description: 'Test',
                prompt: 'Test',
                subagent_type: 'executor'
    ...
    describe('getModelForAgent', () => {
        it('returns correct model for agent with prefix', () => {
            expect(getModelForAgent('brainless:executor')).toBe('sonnet');
            expect(getModelForAgent('brainless:executor-low')).toBe('haiku');
            expect(getModelForAgent('brainless:architect')).toBe('opus');
        });
