...
import { readVerificationState, startVerification, getArchitectVerificationPrompt, clearVerificationState } from './ralph/index.js';
...
import { ULTRAWORK_MESSAGE, ULTRATHINK_MESSAGE, SEARCH_MESSAGE, ANALYZE_MESSAGE, TODO_CONTINUATION_PROMPT, RALPH_MESSAGE } from '../installer/hooks.js';
import { logActivity } from '../features/transcript/index.js';
 ...
 * Logs agent activities to transcript
 */
function processPreToolUse(input) {
    const directory = input.directory || process.cwd();
    const sessionId = input.sessionId || 'cli-session';
    // Track and log Task/Agent tool invocations
    if (input.toolName === 'Task' || input.toolName === 'Agent') {
        const toolInput = input.toolInput;
        if (toolInput?.description && toolInput?.subagent_type) {
            // Generate a pseudo-ID from the description hash
            const taskId = `task-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
            // Log to HUD background tasks
            ...
            try {
                logActivity({
                    ...
                    action: 'spawned',
                    input_summary: toolInput.description,
                    metadata: {
                        task_id: taskId,
                        tool_name: input.toolName,
                        has_prompt: !!toolInput.prompt
                    }
                }, directory);
            }
            catch (err) {
                ...
                if (process.env.AVK_DEBUG === 'true') {
                    console.warn('[AVK] Transcript log error:', err);
                }
            }
        }
    }
 ...
 * Logs completion to transcript
 */
function processPostToolUse(input) {
    const directory = input.directory || process.cwd();
    const sessionId = input.sessionId || 'cli-session';
    // Track and log Task/Agent tool completion
    if (input.toolName === 'Task' || input.toolName === 'Agent') {
        const toolInput = input.toolInput;
        if (toolInput?.subagent_type) {
            ...
            try {
                ...
                const hasError = typeof output === 'object' && output?.error;
                const outputSummary = typeof output === 'string'
                    ? output.slice(0, 200)
                    : typeof output === 'object' && output?.result
                        ? String(output.result).slice(0, 200)
                        : undefined;
                logActivity({
                    session_id: sessionId,
                    agent: toolInput.subagent_type,
                    action: hasError ? 'failed' : 'completed',
                    input_summary: toolInput.description || 'Task completed',
                    output_summary: hasError ? String(output.error) : outputSummary,
                    metadata: {
                        tool_name: input.toolName
                    }
                }, directory);
    ...
    // Capture for Memory Layer - ONLY executor agent work (not orchestration)
    if (input.toolName && input.toolOutput) {
        const toolInput = input.toolInput;
        const isExecutorWork = toolInput?.subagent_type === 'executor' ||
            toolInput?.subagent_type?.includes('executor-') ||
            toolInput?.subagent_type?.includes('architect') ||
            toolInput?.subagent_type?.includes('security-reviewer') ||
            toolInput?.subagent_type?.includes('build-fixer') ||
            toolInput?.subagent_type?.includes('qa-tester');
        // Only record actual implementation/analysis work, not PM orchestration
        if (isExecutorWork) {
            const projectName = path.basename(directory);
            memory.recordInteraction(input.toolName, input.toolInput, input.toolOutput, directory, projectName).catch(err => {
...
function processAutopilot(input) {
    ...
    const context = {
        ...
        specPath: state.expansion.spec_path || '.brainless/autopilot/spec.md',
        planPath: state.planning.plan_path || '.brainless/plans/autopilot-impl.md'
    };
