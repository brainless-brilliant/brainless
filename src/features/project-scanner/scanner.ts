...
export function scanProject(
      ...
      if (depMatch) {
        ...
        if (deps) {
          deps.forEach(dep => {
            ...
            if (pkgName) pythonPackages.push(pkgName);
          });
        }
      }
    } catch {
  ...
  const technologies: DetectedTechnology[] = Array.from(detectionMap.entries())
    .map(([name, data]) => ({
      name,
 ...
 */
function countMatchingFiles(files: Set<string>, pattern: string): number {
 ...
 */
export function formatScanSummary(result: ProjectScanResult): string {
  const lines: string[] = [];
 ...
 */
export function hasTechnology(result: ProjectScanResult, name: string): boolean {
