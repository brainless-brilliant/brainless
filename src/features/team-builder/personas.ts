 ...
 */
export function getPersona(agentId: string): AgentPersona | null {
 ...
 */
export function formatTeamIntro(agentIds: string[]): string {
  const personas = agentIds.map(id => getPersona(id)).filter(Boolean) as AgentPersona[];
 ...
 */
export function getWittyMessage(event: 'team-assembly' | 'memory-search' | 'fallback' | 'haiku-success' | 'api-error'): string {
  const messages = {
    'team-assembly': [
