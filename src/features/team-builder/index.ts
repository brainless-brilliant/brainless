...
import { formatTeamIntro, getWittyMessage, getPersona } from './personas.js';
...
type AgentName = keyof typeof AGENT_REGISTRY;
...
export async function buildTeam(
  ...
  if (classification.recommendedAgents && classification.recommendedAgents.length > 0) {
    ...
    if (validAgents.length > 0) {
      if (verbose) console.log(getWittyMessage('haiku-success'));
      agents = validAgents;
  ...
  try {
    ...
    if (keywords.length > 0) {
      if (verbose) console.log(getWittyMessage('memory-search'));
  ...
  if (agents.length === 0) {
    if (verbose) console.log(getWittyMessage('fallback'));
 ...
 */
function selectTeamFromCapabilities(classification: ClassificationResult): string[] {
  const team: string[] = [];
...
export function getAgentInfo(agentName: string) {
  return AGENT_REGISTRY[agentName as AgentName] || null;
}

/**
 * List all available agents
 */
export function listAvailableAgents(): Array<{ name: string; tier: string; specialty: string }> {
