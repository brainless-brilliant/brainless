...
export interface EnforcementResult {
  ...
  model: ModelType;
  /** Warning message (only if AVK_DEBUG=true) */
  warning?: string;
}
...
export function enforceModel(agentInput: AgentInput): EnforcementResult {
  ...
  // Extract agent type (strip brainless: prefix if present)
  const agentType = agentInput.subagent_type.replace(/^brainless:/, '');
  ...
  // Create warning message (only shown if AVK_DEBUG=true)
  let warning: string | undefined;
  if (process.env.AVK_DEBUG === 'true') {
    warning = `[AVK] Auto-injecting model: ${sdkModel} for ${agentType}`;
  }
 ...
 *
 * @param agentType - The agent type (with or without brainless: prefix)
...
export function getModelForAgent(agentType: string): ModelType {
  const normalizedType = agentType.replace(/^brainless:/, '');
  const agentDefs = getAgentDefinitions();
