 ...
 * - Local state: .brainless/state/{name}.json
 * - Global state: ~/.brainless/state/{name}.json
 *
 * Features:
...
const LOCAL_STATE_DIR = '.brainless/state';
const GLOBAL_STATE_DIR = path.join(os.homedir(), '.anv', 'state');
...
const LEGACY_LOCATIONS: Record<string, string[]> = {
  ...
  'boulder': ['.brainless/boulder.json', path.join(os.homedir(), '.anv', 'boulder.json')],
  'autopilot': ['.brainless/autopilot-state.json'],
  'ralph': ['.brainless/ralph-state.json'],
  'ultrawork': ['.brainless/ultrawork-state.json'],
  'ultraqa': ['.brainless/ultraqa-state.json']
};
