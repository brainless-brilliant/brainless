 ...
 */
export const TECH_TO_AGENT_MAPPINGS: TechToAgentMapping[] = [
  // Languages
  {
...
export function getAgentsByCapability(capability: string): AgentMetadata[] {
  return AGENT_REGISTRY.filter(a =>
    a.capabilities.includes(capability as AgentMetadata['capabilities'][number])
  );
}
 ...
 */
export function getAgentsForTechnology(tech: string): string[] {
 ...
 */
export function getSkillsForTechnology(tech: string): string[] {
  const mapping = TECH_TO_AGENT_MAPPINGS.find(m => m.technology === tech);
