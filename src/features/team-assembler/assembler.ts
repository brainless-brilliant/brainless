...
import type { TeamComposition, TeamAssemblerConfig, AgentMetadata } from './types.js';
import {
  AGENT_REGISTRY,
...
const DEFAULT_CONFIG: Required<TeamAssemblerConfig> = {
  maxAgents: 10,
  minRelevance: 0.3,
...
export function assembleTeam(
  ...
  config?: TeamAssemblerConfig
): TeamComposition {
  const cfg = { ...DEFAULT_CONFIG, ...config };

  // Start with core agents if enabled
  const coreAgents: AgentRecommendation[] = cfg.includeCoreAgents
  ...
  const skillsToInject = new Set<string>();
  ...
  const allRecommendations: AgentRecommendation[] = Array.from(agentScores.entries())
  ...
  const techAgents = allRecommendations.filter(
    a => !CORE_AGENT_NAMES.includes(a.agentName)
  );
 ...
 */
export function formatResourceSheet(composition: TeamComposition): string {
  const lines: string[] = [];
...
export function getTeamForTask(
  ...
  taskType: 'planning' | 'implementation' | 'review' | 'testing'
): AgentRecommendation[] {
  ...
  return composition.allAgents.filter(agent => {
    ...
    return metadata.capabilities.some(cap =>
