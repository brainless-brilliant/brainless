 ...
 */
export function getDebatesDir(workingDir?: string): string {
 ...
 */
export function ensureDebatesDir(workingDir?: string): void {
 ...
 * Generate concern ID
 */
export function generateConcernId(): string {
...
export function createDebate(
  ...
  const room: DebateRoom = {
    ...
    participants: participants.map(p => 
      p.startsWith('brainless:') ? p : `brainless:${p}`
    ),
 ...
 */
export function saveDebate(room: DebateRoom, workingDir?: string): void {
  ensureDebatesDir(workingDir);
 ...
 * Load a debate room
 */
export function loadDebate(debateId: string, workingDir?: string): DebateRoom | null {
  ...
  try {
    ...
    return JSON.parse(content) as DebateRoom;
  } catch {
    return null;
  }
}
 ...
 * List all debates
 */
export function listDebates(workingDir?: string): DebateSummary[] {
  const dir = getDebatesDir(workingDir);
 ...
 * Get debate summary
 */
export function getDebateSummary(room: DebateRoom): DebateSummary {
...
export function addCounter(
  ...
  return addMessage(
    ...
    'counter', 
    counter, 
    referencedProposalId ? [referencedProposalId] : undefined,
    undefined,
    undefined,
    workingDir
  );
...
export function raiseConcern(
  ...
  const concern: CrossCuttingConcern = {
  ...
  addMessage(
    ...
    from,
    'concern',
    `⚠️ **Cross-cutting concern**: ${description}\n\n**Affects**: ${affects.join(', ')}\n\n**Priority**: ${priority}`,
    undefined,
    affects,
    priority,
    workingDir
 ...
 */
export function advanceRound(debateId: string, workingDir?: string): DebateRoom | null {
  const room = loadDebate(debateId, workingDir);
 ...
 */
export function formatDebate(debateId: string, workingDir?: string): string {
  const room = loadDebate(debateId, workingDir);
 ...
 */
function getMessageIcon(type: DebateMessageType): string {
  const icons: Record<DebateMessageType, string> = {
    proposal: '💡',
    counter: '↩️',
    support: '👍',
    question: '❓',
 ...
 */
export function getActiveConcerns(debateId: string, workingDir?: string): CrossCuttingConcern[] {
  const room = loadDebate(debateId, workingDir);
 ...
 */
export function getBlockers(debateId: string, workingDir?: string): CrossCuttingConcern[] {
