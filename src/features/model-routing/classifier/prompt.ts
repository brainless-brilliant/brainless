 ...
 */
export const CLASSIFICATION_SYSTEM_PROMPT = `You are a task classifier for an AI coding assistant. Your job is to analyze a task description and determine what capabilities are needed to complete it.
 ...
 */
export function createClassificationPrompt(taskPrompt: string): string {
 ...
 */
export function createSystemPromptWithExamples(): string {
  const examples = CLASSIFICATION_EXAMPLES.map((ex, i) => 
    `Example ${i + 1}:
Task: "${ex.task}"
