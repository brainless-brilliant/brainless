...
/**
 * Get the HUD state file path in the project's .anv directory
 */
...
function getConfigFilePath(): string {
  return join(homedir(), '.claude', '.anv', 'hud-config.json');
}
...
/**
 * Ensure the ~/.claude/.anv directory exists
 */
function ensureGlobalConfigDir(): void {
  const configDir = join(homedir(), '.claude', '.anv');
