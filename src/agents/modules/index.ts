 ...
 */
export const MODULES_BY_ID: Record<PromptModuleId, PromptModule> = {
 ...
 */
export function selectModules(capabilities: TaskCapabilities): PromptModule[] {
  return ALL_MODULES.filter(
    (module) => module.alwaysInclude || module.shouldInclude(capabilities)
  );
}
 ...
 */
export function composePromptFromModules(
  ...
  const parts: string[] = [];
  const includedModules: PromptModuleId[] = [];
  let totalTokens = 0;
 ...
 */
export function getDefaultCapabilities(): TaskCapabilities {
  return {
