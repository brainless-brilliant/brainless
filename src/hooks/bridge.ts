...
import { logActivity, type AgentAction } from '../features/transcript/index.js';
...
function processPostToolUse(input: HookInput): HookOutput {
  ...
  if (input.toolName === 'Task' || input.toolName === 'Agent') {
    ...
    if (toolInput?.subagent_type) {
      ...
      try {
        const output = input.toolOutput as { result?: string; error?: string } | string | undefined;
        ...
        logActivity({
          ...
          output_summary: hasError ? String((output as { error: string }).error) : outputSummary,
          metadata: {
            tool_name: input.toolName
          }
        }, directory);
  ...
  if (input.toolName && input.toolOutput) {
    const toolInput = input.toolInput as { subagent_type?: string } | undefined;
