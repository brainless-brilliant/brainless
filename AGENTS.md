...
└── .brainless/                    # Runtime state directory
    ├── plans/               # Planner plans
...
- **State**: JSON files in `~/.claude/.brainless/`
...
| File | Purpose |
|------|---------|
| `~/.claude/.brainless/plan-state.json` | Active plan state |
| `~/.claude/.brainless/ralph.json` | Ralph Loop state |
| `~/.claude/.brainless/rules-injector/*.json` | Injected rules tracking |
| `~/.claude/.brainless/background-tasks/*.json` | Background task state |
