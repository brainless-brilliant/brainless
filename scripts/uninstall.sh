...
rm -f "$CLAUDE_CONFIG_DIR/agents/anveeksha-junior.md"
...
rm -f "$CLAUDE_CONFIG_DIR/.brainless-version.json"
rm -f "$CLAUDE_CONFIG_DIR/.brainless-silent-update.json"
rm -f "$CLAUDE_CONFIG_DIR/.brainless-update.log"
rm -f "$CLAUDE_CONFIG_DIR/.brainless-config.json"
...
# Remove .anv directory if it exists (plans, notepads, drafts)
if [ -d "$CLAUDE_CONFIG_DIR/../.anv" ] || [ -d ".anv" ]; then
    echo -e "${YELLOW}Note: .anv directory (plans/notepads) was not removed.${NC}"
