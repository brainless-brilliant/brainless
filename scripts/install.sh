...
echo -e "\033[31m╚═══════════════════════════════════════════════════════════╝\033[0m"
echo ""
echo -e "\033[33mAs of v3.0.0, anveekshacode is installed via the Claude Code plugin system only.\033[0m"
echo ""
echo -e "\033[32mTo install:\033[0m"
echo "  /plugin marketplace add Yeachan-Heo/anveekshacode"
echo ""
echo -e "\033[32mThen run setup:\033[0m"
echo "  /anveekshacode:omc-setup"
echo ""
echo -e "\033[34mFull documentation: https://yeachan-heo.github.io/anveekshacode-website\033[0m"
echo ""
exit 0
...
2. **Plan Following**: Read and follow plans from `.brainless/plans/`
3. **Learning Recording**: Document learnings in `.brainless/notepads/`
...
4. **Plan Storage**: Save plans to `.brainless/plans/{name}.md`
...
6. **Draft Storage** - Save drafts to `.brainless/drafts/{name}.md` during iteration
...
| Execution | anveeksha-junior-low | anveeksha-junior | anveeksha-junior-high |
...
---
description: Configure anveekshacode in local project (.claude/CLAUDE.md)
---

$ARGUMENTS

## Task: Configure anveekshacode Default Mode (Project-Scoped)
...
curl -fsSL "https://raw.githubusercontent.com/Yeachan-Heo/anveeksha/main/docs/CLAUDE.md" -o .claude/CLAUDE.md && \
...
https://raw.githubusercontent.com/Yeachan-Heo/anveeksha/main/docs/CLAUDE.md

### Step 3: Verify Plugin Installation

The anveekshacode plugin provides all hooks automatically via the plugin system. Verify the plugin is enabled:

```bash
grep -q "anveekshacode" ~/.claude/settings.json && echo "Plugin enabled" || echo "Plugin NOT enabled"
```

If plugin is not enabled, instruct user:
> Run: `claude /install-plugin anveekshacode` to enable the plugin.

### Step 4: Confirm Success

After completing all steps, report:

✅ **anveekshacode Project Configuration Complete**
...
After installing anveekshacode updates (via npm or plugin update), run `/omc-default` again in your project to get the latest CLAUDE.md configuration. This ensures you have the newest features and agent configurations.
...
---
description: Configure anveekshacode globally in ~/.claude/CLAUDE.md
---

$ARGUMENTS

## Task: Configure anveekshacode Default Mode (Global)
...
✅ **anveekshacode Global Configuration Complete**
- CLAUDE.md: Updated with latest configuration from GitHub at ~/.claude/CLAUDE.md
...
After installing anveekshacode updates (via npm or plugin update), run `/omc-default-global` again to get the latest CLAUDE.md configuration. This ensures you have the newest features and agent configurations.
CMD_EOF
...
### Plan Storage
- Drafts are saved to `.brainless/drafts/`
- Final plans are saved to `.brainless/plans/`
...
### Usage
```
/review .brainless/plans/my-feature.md
...
Provide a plan file path to review, or I'll review the most recent plan in `.brainless/plans/`.
CMD_EOF
...
CONFIG_FILE="$CLAUDE_CONFIG_DIR/.brainless-config.json"
ENABLE_SILENT_UPDATE="false"

echo ""
echo -e "${YELLOW}Silent Auto-Update Configuration${NC}"
echo "  anveekshacode can automatically check for and install updates in the background."
...
#!/bin/bash
# anveekshacode Keyword Detector Hook
...
#!/bin/bash
# anveekshacode Stop Continuation Hook
...
#!/bin/bash
# anveekshacode Silent Auto-Update Hook
...
# silent auto-updates in ~/.claude/.brainless-config.json
#
...
(
  # Configuration
  CONFIG_FILE="$HOME/.claude/.brainless-config.json"
  VERSION_FILE="$HOME/.claude/.brainless-version.json"
  STATE_FILE="$HOME/.claude/.brainless-silent-update.json"
  LOG_FILE="$HOME/.claude/.brainless-update.log"
  CHECK_INTERVAL_HOURS=24
  REPO_URL="https://raw.githubusercontent.com/Yeachan-Heo/anveeksha/main"
...
EOF
  ...
  LOCK_FILE="$HOME/.claude/.brainless-update.lock"
...
VERSION="2.0.6"
VERSION_FILE="$CLAUDE_CONFIG_DIR/.brainless-version.json"
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║         anveekshacode Installation Complete!           ║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo "  anveeksha-junior     - Focused execution (Sonnet)"
...
echo "  anveeksha-junior-low, -high         - Simple to complex execution"
...
echo "  curl -fsSL https://raw.githubusercontent.com/Yeachan-Heo/anveeksha/main/scripts/install.sh | bash"
echo ""
