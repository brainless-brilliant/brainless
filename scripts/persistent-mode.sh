...
ULTRAWORK_STATE=""
if [ -f "$DIRECTORY/.brainless/ultrawork-state.json" ]; then
  ULTRAWORK_STATE=$(cat "$DIRECTORY/.brainless/ultrawork-state.json" 2>/dev/null)
...
RALPH_STATE=""
if [ -f "$DIRECTORY/.brainless/ralph-state.json" ]; then
  RALPH_STATE=$(cat "$DIRECTORY/.brainless/ralph-state.json" 2>/dev/null)
fi

# Check for verification state (oracle verification)
VERIFICATION_STATE=""
if [ -f "$DIRECTORY/.brainless/ralph-verification.json" ]; then
  VERIFICATION_STATE=$(cat "$DIRECTORY/.brainless/ralph-verification.json" 2>/dev/null)
fi
...
for todo_path in "$DIRECTORY/.brainless/todos.json" "$DIRECTORY/.claude/todos.json"; do
  if [ -f "$todo_path" ]; then
    if command -v jq &> /dev/null; then
...
EOF
    ...
    if [ "$ITERATION" -lt "$MAX_ITER" ]; then
      ...
      echo "$RALPH_STATE" | jq ".iteration = $NEW_ITER" > "$DIRECTORY/.brainless/ralph-state.json" 2>/dev/null
...
if [ -n "$ULTRAWORK_STATE" ] && [ "$INCOMPLETE_COUNT" -gt 0 ]; then
  ...
  if [ "$IS_ACTIVE" = "true" ]; then
    ...
    if command -v jq &> /dev/null; then
      echo "$ULTRAWORK_STATE" | jq ".reinforcement_count = $NEW_COUNT | .last_checked_at = \"$(date -Iseconds)\"" > "$DIRECTORY/.brainless/ultrawork-state.json" 2>/dev/null
    fi
