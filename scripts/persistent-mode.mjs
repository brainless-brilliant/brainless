...
function getProgressPatterns(projectDir) {
  const paths = [
    ...
    join(projectDir, '.anv', 'progress.txt')
  ];
...
function countIncompleteTodos(todosDir, projectDir) {
  ...
  // Check project todos
  for (const path of [
    join(projectDir, '.anv', 'todos.json'),
...
async function main() {
  try {
    ...
    let ultraworkState = readJsonFile(join(directory, '.anv', 'ultrawork-state.json'))
      || readJsonFile(join(homedir(), '.claude', 'ultrawork-state.json'));

    // Check for ralph loop state
    const ralphState = readJsonFile(join(directory, '.anv', 'ralph-state.json'));

    // Check for verification state
    const verificationState = readJsonFile(join(directory, '.anv', 'ralph-verification.json'));
...
`
      ...
      if (iteration < maxIter) {
        ...
        writeJsonFile(join(directory, '.anv', 'ralph-state.json'), ralphState);
...
`
    ...
    if (ultraworkState?.active && incompleteCount > 0) {
      ...
      writeJsonFile(join(directory, '.anv', 'ultrawork-state.json'), ultraworkState);
...
`
    ...
    if (incompleteCount > 0) {
      ...
      const contFile = join(directory, '.anv', 'continuation-count.json');
