...
function checkHudInstallation() {
  const hudScript = join(homedir(), '.claude', 'hud', 'anveeksha-hud.mjs');
...
[Brainless] HUD not configured (${hudCheck.reason}). Run /hud setup then restart Claude Code.
    ...
    const ultraworkState = readJsonFile(join(directory, '.anv', 'ultrawork-state.json'))
...
`);
    ...
    const notepadPath = join(directory, '.anv', 'notepad.md');
