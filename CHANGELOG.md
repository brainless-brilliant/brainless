...
- **CRITICAL: CLAUDE.md Template** - Complete rewrite from user-facing README to behavioral instructions
  - Previous template explained WHAT the plugin does (for human readers)
  - New template instructs Claude on HOW to behave (mandatory rules)
  - Added MUST/NEVER imperatives for: team assembly, delegation, escalation, memory
  - Template now contains actual behavioral protocol instead of feature descriptions
  - **Impact**: Plugin will now actually invoke team assembly automatically instead of just mentioning it exists

### Technical Details

The template now contains:
- **Automatic Team Assembly Protocol** - When to trigger, how to invoke, mandatory display format
- **Specialist Registry** - All 29 agents with their domains
- **3-Tier Escalation Rules** - Self → Specialist → User (with examples)
- **Memory Integration** - Search before work, capture after completion
- **Delegation Protocol** - NEVER do specialist work, ALWAYS delegate

**This was a critical bug** - the plugin effectively did nothing because Claude had no instructions to follow.
...
- **Ultrapilot**: Parallel autopilot with up to 5 concurrent workers
  ...
  - State tracking in `.brainless/state/ultrapilot-state.json` and `.brainless/state/ultrapilot-ownership.json`
...
- **Ecomode**: Token-efficient parallel execution mode
  ...
  - State tracking in `.brainless/ecomode-state.json`
...
- **State Management Module** (`src/features/state-manager/`)
  - Standardized state locations: `.brainless/state/{name}.json` (local), `~/.brainless/state/{name}.json` (global)
...
- **PreToolUse hooks**: Soft enforcement for delegation via `pre-tool-use.sh` and `pre-tool-use.mjs`
  ...
  - Path-based exceptions for `.brainless/`, `.claude/`, `CLAUDE.md`, `AGENTS.md`
