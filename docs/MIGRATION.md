...
2. **Verify directories exist**:
   ```bash
   ls -la .brainless/  # In project directory
   ls -la ~/.brainless/  # Global directory
   ```
...
Storage:
- **Project-level**: `.brainless/skills/` (version-controlled)
...
**Location:** `.brainless/notepads/{plan-name}/`
...
- State files: `.brainless/state/ultrapilot-state.json`, `.brainless/state/ultrapilot-ownership.json`
...
**Standard paths:**
- Local: `.brainless/state/{name}.json`
- Global: `~/.brainless/state/{name}.json`
...
Set your preferred execution mode in `~/.claude/.brainless-config.json`:
...
4. **Check state directory**:
   ```bash
   ls -la .brainless/state/  # Should see ultrapilot-state.json after running ultrapilot
   ```
