---
description: Cancel active Ecomode mode (deprecated - use /anveekshacode:cancel instead)
---
...
**DEPRECATION NOTICE:** This command is deprecated. Use `/anveekshacode:cancel` instead, which intelligently detects and cancels any active mode including ecomode.
...
2. If linked → Warn and suggest using `/anveekshacode:cancel-ralph`
...
if [[ "$LINKED" == "true" ]]; then
  ...
  echo "Use /anveekshacode:cancel to cancel both modes."
  exit 1
fi
