...
Ralplan maintains persistent state in `.brainless/ralplan-state.json` to track progress and enable recovery across interruptions:
...
```json
{
  ...
  "iteration": 1,
  "max_iterations": 5,
  "plan_path": ".brainless/plans/[feature].md",
...
1. Create `.brainless/plans/` directory if it doesn't exist
...
- Planner generates plan directly to `.brainless/plans/[feature-name].md`
...
- Signal completion with `PLAN_READY: .brainless/plans/[filename].md`
