...
```
/anveekshacode:autopilot <your idea>
/anveekshacode:ap "A CLI tool that tracks daily habits"
/anveekshacode:autopilot Add dark mode to the app
```
...
**Output:** `.brainless/autopilot/spec.md`
...
**Output:** `.brainless/plans/autopilot-impl.md`
...
```
/anveekshacode:cancel-autopilot
```
...
If autopilot was cancelled or failed, just run `/anveekshacode:autopilot` again to resume from where it stopped.

## Examples

**New Project:**
```
/anveekshacode:autopilot A REST API for a bookstore inventory with CRUD operations
```

**Feature Addition:**
```
/anveekshacode:autopilot Add user authentication with JWT tokens
```

**Enhancement:**
```
/anveekshacode:ap Add dark mode support with system preference detection
```
...
- Review `.brainless/autopilot-state.json` for state
