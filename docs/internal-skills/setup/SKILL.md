---
name: setup
description: Setup and configure Brainless AI Workforce (the ONLY command you need to learn)
---
...
OLD_VERSION=$(grep -m1 "^# anveeksha" .claude/CLAUDE.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "none")
...
NEW_VERSION=$(grep -m1 "^# anveeksha" .claude/CLAUDE.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
...
```bash
grep -q "anveeksha" ~/.claude/settings.json && echo "Plugin verified" || echo "Plugin NOT found - run: claude /install-plugin @anveeksha/workforce"
```
...
This will:
1. Install the HUD wrapper script to `~/.claude/hud/avk-hud.mjs`
...
CACHE_DIR="$HOME/.claude/plugins/cache/anveeksha/workforce"
if [ -d "$CACHE_DIR" ]; then
...
if [ -d "$HOME/.claude/plugins/cache/anveeksha/workforce" ]; then
  INSTALLED_VERSION=$(ls -1 "$HOME/.claude/plugins/cache/anveeksha/workforce" | sort -V | tail -1)
fi

# Try .avk-version.json second
if [ -z "$INSTALLED_VERSION" ] && [ -f ".avk-version.json" ]; then
  INSTALLED_VERSION=$(grep -oE '"version":\s*"[^"]+' .avk-version.json | cut -d'"' -f4)
fi

# Try CLAUDE.md header third (local first, then global)
if [ -z "$INSTALLED_VERSION" ]; then
  if [ -f ".claude/CLAUDE.md" ]; then
    INSTALLED_VERSION=$(grep -m1 "^# anveeksha" .claude/CLAUDE.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sed 's/^v//')
  elif [ -f "$HOME/.claude/CLAUDE.md" ]; then
    INSTALLED_VERSION=$(grep -m1 "^# anveeksha" "$HOME/.claude/CLAUDE.md" 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sed 's/^v//')
  fi
fi

# Check npm for latest version
LATEST_VERSION=$(npm view @anveeksha/workforce version 2>/dev/null)
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  ...
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin @anveeksha/workforce"
  else
...
Store the preference in `~/.claude/.avk-config.json`:

```bash
# Read existing config or create empty object
CONFIG_FILE="$HOME/.claude/.avk-config.json"
...
## Step 6: Detect Upgrade from Previous Version

Check if user has existing configuration:
```bash
# Check for existing 2.x/3.x artifacts
ls ~/.claude/commands/ralph-loop.md 2>/dev/null || ls ~/.claude/commands/ultrawork.md 2>/dev/null
```

If found, this is an upgrade from a previous version.
...
HUD STATUSLINE:
The status bar now shows Brainless state. Restart Claude Code to see it.
...
```
Brainless Setup Complete! (Upgraded)
...
**Question:** "If you're enjoying Brainless AI Workforce, would you like to support the project by starring it on GitHub?"
...
```bash
gh api -X PUT /user/starred/anveeksha/workforce 2>/dev/null && echo "Thanks for starring! ⭐" || true
```
...
```bash
echo ""
echo "If you enjoy Brainless AI Workforce, consider starring the repo:"
echo "  https://github.com/anveeksha/workforce"
echo ""
```

## Keeping Up to Date

After installing Brainless updates (via npm or plugin update), run:
- `/brainless:setup --local` to update project config
- `/brainless:setup --global` to update global config

This ensures you have the newest features and agent configurations.

## Help Text

When user runs `/brainless:setup --help` or just `--help`, display:

```
Brainless Setup - Configure Brainless AI Workforce

USAGE:
  /brainless:setup           Run initial setup wizard
  /brainless:setup --local   Configure local project (.claude/CLAUDE.md)
  /brainless:setup --global  Configure global settings (~/.claude/CLAUDE.md)
  /brainless:setup --help    Show this help
...
MODES:
  ...
  Local Configuration (--local)
    ...
    - Use this to update project config after upgrades

  Global Configuration (--global)
    - Downloads fresh CLAUDE.md to ~/.claude/
    - Applies to all Claude Code sessions
    - Cleans up legacy hooks
    - Use this to update global config after upgrades

EXAMPLES:
  /brainless:setup           # First time setup
  /brainless:setup --local   # Update this project
  /brainless:setup --global  # Update all projects

For more info: https://github.com/anveeksha/workforce
