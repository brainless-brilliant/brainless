...
- `.brainless/autopilot-state.json` → Autopilot detected
- `.brainless/ralph-state.json` → Ralph detected
- `.brainless/ultrawork-state.json` → Ultrawork detected
- `.brainless/ecomode-state.json` → Ecomode detected
- `.brainless/ultraqa-state.json` → UltraQA detected
- `.brainless/swarm-state.json` → Swarm detected
- `.brainless/ultrapilot-state.json` → Ultrapilot detected
- `.brainless/pipeline-state.json` → Pipeline detected
...
- `.brainless/autopilot-state.json`
- `.brainless/ralph-state.json`
- `.brainless/ultrawork-state.json`
- `.brainless/ecomode-state.json`
- `.brainless/ultraqa-state.json`
- `.brainless/swarm-state.json`
- `.brainless/state/swarm-tasks.json`
- `.brainless/ultrapilot-state.json`
- `.brainless/pipeline-state.json`
...
if [[ -f .brainless/autopilot-state.json ]]; then
  AUTOPILOT_ACTIVE=$(cat .brainless/autopilot-state.json | jq -r '.active // false')
fi

if [[ -f .brainless/ralph-state.json ]]; then
  RALPH_ACTIVE=$(cat .brainless/ralph-state.json | jq -r '.active // false')
fi

if [[ -f .brainless/ultrawork-state.json ]]; then
  ULTRAWORK_ACTIVE=$(cat .brainless/ultrawork-state.json | jq -r '.active // false')
fi

if [[ -f .brainless/ecomode-state.json ]]; then
  ECOMODE_ACTIVE=$(cat .brainless/ecomode-state.json | jq -r '.active // false')
fi

if [[ -f .brainless/ultraqa-state.json ]]; then
  ULTRAQA_ACTIVE=$(cat .brainless/ultraqa-state.json | jq -r '.active // false')
fi
```
...
if [[ "$FORCE_MODE" == "true" ]]; then
  ...
  rm -f .brainless/autopilot-state.json
  rm -f .brainless/ralph-state.json
  rm -f .brainless/ultrawork-state.json
  rm -f .brainless/ecomode-state.json
  rm -f .brainless/ultraqa-state.json
  rm -f .brainless/ralph-plan-state.json
  rm -f .brainless/ralph-verification.json
  rm -f .brainless/swarm-state.json
  rm -f .brainless/state/swarm-tasks.json
  rm -f .brainless/ultrapilot-state.json
  rm -f .brainless/pipeline-state.json
...
if [[ -f .brainless/autopilot-state.json ]]; then
  ...
  if [[ -f .brainless/ralph-state.json ]]; then
    RALPH_STATE=$(cat .brainless/ralph-state.json)
    LINKED_UW=$(echo "$RALPH_STATE" | jq -r '.linked_ultrawork // false')

    # Clean linked ultrawork first
    if [[ "$LINKED_UW" == "true" ]] && [[ -f .brainless/ultrawork-state.json ]]; then
      rm -f .brainless/ultrawork-state.json
      rm -f ~/.claude/ultrawork-state.json
  ...
  CURRENT_STATE=$(cat .brainless/autopilot-state.json)
  CURRENT_PHASE=$(echo "$CURRENT_STATE" | jq -r '.phase // "unknown"')
  echo "$CURRENT_STATE" | jq '.active = false' > .brainless/autopilot-state.json

  echo "Autopilot cancelled at phase: $CURRENT_PHASE. Progress preserved for resume."
  echo "Run /anveekshacode:autopilot to resume."
fi
```
...
if [[ -f .brainless/ralph-state.json ]]; then
  ...
  # Clean linked ultrawork first
  if [[ "$LINKED_UW" == "true" ]] && [[ -f .brainless/ultrawork-state.json ]]; then
    UW_STATE=$(cat .brainless/ultrawork-state.json)
...
if [[ -f .brainless/ultrawork-state.json ]]; then
  ...
  if [[ "$LINKED" == "true" ]]; then
    echo "Ultrawork is linked to Ralph. Use /anveekshacode:cancel to cancel both."
    exit 1
  fi
...
echo ""
echo "Checked for:"
echo "  - Autopilot (.brainless/autopilot-state.json)"
echo "  - Ralph (.brainless/ralph-state.json)"
echo "  - Ultrawork (.brainless/ultrawork-state.json)"
echo "  - UltraQA (.brainless/ultraqa-state.json)"
echo ""
...
if [[ -f .brainless/autopilot-state.json ]]; then
  AUTOPILOT_STATE=$(cat .brainless/autopilot-state.json)
  ...
  if [[ "$AUTOPILOT_ACTIVE" == "true" ]]; then
    ...
    # Clean up ultraqa if active
    if [[ -f .brainless/ultraqa-state.json ]]; then
      ULTRAQA_STATE=$(cat .brainless/ultraqa-state.json)
    ...
    echo "$AUTOPILOT_STATE" | jq '.active = false' > .brainless/autopilot-state.json
    ...
    echo "Progress preserved for resume. Run /anveekshacode:autopilot to continue."
...
if [[ -f .brainless/ralph-state.json ]]; then
  ...
  if [[ "$RALPH_ACTIVE" == "true" ]]; then
    ...
    if [[ "$LINKED_ECO" == "true" ]] && [[ -f .brainless/ecomode-state.json ]]; then
      ECO_STATE=$(cat .brainless/ecomode-state.json)
...
if [[ -f .brainless/swarm-state.json ]]; then
  SWARM_STATE=$(cat .brainless/swarm-state.json)
...
if [[ -f .brainless/ultrapilot-state.json ]]; then
  ULTRAPILOT_STATE=$(cat .brainless/ultrapilot-state.json)
...
if [[ -f .brainless/pipeline-state.json ]]; then
  PIPELINE_STATE=$(cat .brainless/pipeline-state.json)
...
if [[ "$CANCELLED_ANYTHING" == "false" ]]; then
  ...
  echo "  - Ralph (.brainless/ralph-state.json)"
  echo "  - Ultrawork (.brainless/ultrawork-state.json)"
  echo "  - Ecomode (.brainless/ecomode-state.json)"
  echo "  - UltraQA (.brainless/ultraqa-state.json)"
  echo "  - Swarm (.brainless/swarm-state.json)"
  echo "  - Ultrapilot (.brainless/ultrapilot-state.json)"
  echo "  - Pipeline (.brainless/pipeline-state.json)"
  echo ""
  echo "Use --force to clear all state files anyway."
fi
```
...
|------|-----------------|----------------|
| Autopilot | Yes (phase, files, spec, plan, verdicts) | `/anveekshacode:autopilot` |
| Ralph | No | N/A |
| Ultrawork | No | N/A |
| UltraQA | No | N/A |
| Swarm | No | N/A |
...
- **Dual-location**: Clears both `.brainless/` and `~/.claude/` state files
