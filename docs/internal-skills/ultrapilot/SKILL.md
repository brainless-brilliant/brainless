...
```
/anveekshacode:ultrapilot <your task>
/anveekshacode:up "Build a full-stack todo app"
/anveekshacode:ultrapilot Refactor the entire backend
```
...
**Output:** `.brainless/ultrapilot/decomposition.json`
...
**Data Structure:** `.brainless/state/ultrapilot-ownership.json`
...
**Location:** `.brainless/ultrapilot-state.json`
...
**Location:** `.brainless/state/ultrapilot-ownership.json`

Tracks which worker owns which files (see Phase 2 example above).

### Progress Tracking

**Location:** `.brainless/ultrapilot/progress.json`
...
```
/anveekshacode:cancel-ultrapilot
```
...
```
/anveekshacode:ultrapilot resume
```
...
```
/anveekshacode:ultrapilot Build a todo app with React frontend, Express backend, and PostgreSQL database
```
...
```
/anveekshacode:up Refactor all services to use dependency injection
```
...
```
/anveekshacode:ultrapilot Generate tests for all untested modules
```
...
4. **Monitor progress** - Check `.brainless/ultrapilot/progress.json`
...
- Review `.brainless/ultrapilot/decomposition.json` for details

**Worker hangs?**
- Check worker logs in `.brainless/logs/ultrapilot-worker-N.log`
- Cancel and restart that worker
- May indicate file ownership issue

**Integration conflicts?**
- Review `.brainless/ultrapilot-state.json` conflicts array
...
**Location:** `.brainless/ultrapilot/custom-decomposition.json`
...
Then run:
```
/anveekshacode:ultrapilot --custom-decomposition
```
