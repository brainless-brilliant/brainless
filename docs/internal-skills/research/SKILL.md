...
```
/anveekshacode:research What are the performance characteristics of different sorting algorithms?
/anveekshacode:research AUTO: Analyze authentication patterns in this codebase
/anveekshacode:research How does the error handling work across the API layer?
```
...
Task(subagent_type="anveekshacode:scientist", model="sonnet", prompt="
...
```
/anveekshacode:research AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="anveekshacode:scientist-low", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="anveekshacode:scientist-low", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="anveekshacode:scientist-low", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="anveekshacode:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="anveekshacode:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="anveekshacode:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="anveekshacode:scientist-high", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/anveekshacode:research status` | Show current session progress |
| `/anveekshacode:research resume` | Resume most recent interrupted session |
| `/anveekshacode:research resume <session-id>` | Resume specific session |
| `/anveekshacode:research list` | List all sessions with status |
| `/anveekshacode:research report <session-id>` | Generate/regenerate report |
| `/anveekshacode:research cancel` | Cancel current session (preserves state) |
...
Progress is preserved in `.brainless/research/{session-id}/` for resume.
