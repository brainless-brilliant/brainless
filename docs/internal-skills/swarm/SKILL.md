...
### `.brainless/state/swarm-claims.json`
...
function claimTask() {
  const tasks = readJSON('.brainless/state/swarm-tasks.json');
  ...
  for (const task of tasks.tasks) {
    if (task.status === 'pending') {
      ...
      writeJSON('.brainless/state/swarm-tasks.json', tasks);
      return task;
    }
  }
...
**Shared Task List:** .brainless/state/swarm-tasks.json
