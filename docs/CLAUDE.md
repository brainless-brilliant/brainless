...
| `.brainless/**` | OMC state and config |
...
This is **soft enforcement** (warnings only). Audit log at `.brainless/logs/delegation-audit.jsonl`.
...
**State files:**
- `.brainless/state/ultrapilot-state.json` - Session state
- `.brainless/state/ultrapilot-ownership.json` - File ownership
